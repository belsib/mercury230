...
AUTO_LOAD = ["mercury230"]

Mercury230TextSensor = mercury230_ns.class_(
    "Mercury230TextSensor", text_sensor.TextSensor
)

CONFIG_SCHEMA = cv.All(
    text_sensor.text_sensor_schema(
        Mercury230TextSensor,
    ).extend(
        {
            cv.GenerateID(CONF_MERCURY230_ID): cv.use_id(Mercury230),
...
async def to_code(config):
    component = await cg.get_variable(config[CONF_MERCURY230_ID])
