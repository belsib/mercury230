...
void MercuryComponent::loop() {
  ...
  switch (this->state_) {
    ...
    case State::OPEN_SESSION: {// ----------------------------------------------------------------------------------OPEN_SESSION
	  ...
	  uint8_t open_cmd[4]{0, 0};
	  uint8_t open_cmd_len{4};
    ...
    // case State::OPEN_SESSION_GET_ID:  // ------------------------------------------------------------------------OPEN_SESSION_GET_ID
      // this->log_state_();
    ...
    // case State::ACK_START_GET_INFO: // --------------------------------------------------------------------------ACK_START_GET_INFO
      // this->log_state_();
    ...
    // case State::SINGLE_READ_ACK: {  // --------------------------------------------------------------------------SINGLE_READ_ACK
      // this->log_state_();
      // if (received_frame_size_) {
...
// void MercuryComponent::queue_single_read(const std::string &request) {
...
// #ifdef USE_TIME
// void MercuryComponent::sync_device_time() {
...
// void MercuryComponent::set_device_time(uint32_t timestamp) {
...
bool MercuryComponent::set_sensor_value_(EnergomeraIecSensorBase *sensor, ValueRefsArray &vals) {
...
uint8_t MercuryComponent::calculate_crc_prog_frame_(uint8_t *data, size_t length, bool set_crc) {
  uint8_t crc = 0;
  if (length < 2) {
    return 0;
  }
...
void MercuryComponent::read_reply_and_go_next_state_(ReadFunction read_fn, State next_state, uint8_t retries,
...
// void MercuryComponent::prepare_prog_frame_(const char *request, bool write) {
...
// void MercuryComponent::prepare_non_session_prog_frame_(const char *request) {
  // // we assume request has format "XXXX(params)"
  // // we assume it always has brackets
...
// void MercuryComponent::prepare_ctime_frame_(uint8_t hh, uint8_t mm, uint8_t ss) {
  // // "/?CTIME(HH:MM:SS)!\r\n"
  // // no crc
...
void MercuryComponent::send_frame_prepared_() {
  ...
  // ESP_LOGV(TAG, "TX: %s.%s.%s", format_frame_pretty(this->buffers_.out, this->buffers_.amount_out).c_str());
  ESP_LOGI(TAG, "TX: %s.%s.%s", format_hex_pretty(this->buffers_.out, this->buffers_.amount_out).c_str(), 
                                 format_hex_pretty(crc & 0xFF, 1), 
								 format_hex_pretty((crc >> 8) & 0xFF, 1));
}
...
size_t MercuryComponent::receive_frame_ascii_() {
  // "data<CR><LF>"
...
// size_t MercuryComponent::receive_frame_ack_nack_() {
  // // "<ACK/NAK>"
...
// size_t MercuryComponent::receive_prog_frame_(uint8_t start_byte, bool accept_ack_and_nack) {
...
// char *MercuryComponent::extract_meter_id_(size_t frame_size) {
...
// uint8_t MercuryComponent::get_values_from_brackets_(char *line, ValueRefsArray &vals) {
...
// char *MercuryComponent::get_nth_value_from_csv_(char *line, uint8_t idx) {
  // if (idx == 0) {
    // return line;
  // }
  // char *ptr;
